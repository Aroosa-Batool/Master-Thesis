"""Re-Consent policy demo: always ask, never remember.

Same sensing pipeline as ``demo_cache_memory.py`` - it reads heart rate
from the Bangle.js, watches the webcam for presence, and tells the owner
(watch-wearer) apart from bystanders with SFace - but the disclosure
decision is taken **fresh every time**. There is no consent cache: every
elevated-HR encounter with a bystander triggers a new Yes/No prompt on
the watch, even if the same person answered only a moment ago.

This is the privacy "Re-Consent" baseline that the cache-memory demo is
compared against; the only difference is the absence of the stored-
decision lookup. The user is still **recognised** - bystander IDs are
minted and logged via the shared ``face_db.json`` gallery, and the owner
is filtered out via ``owner_face.json`` - but that identity is used only
for logging here. It never short-circuits the prompt.

As in the main demo, the laptop terminal never takes input: every answer
is given on the watch, and the only laptop interaction is pressing 'q' in
the camera window to quit.
